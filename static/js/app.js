...
app.controller('MainController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.click = function(row, col) {
        ...
        if ($scope.board[row][col] === ' ') {
            ...
            $scope.inactive = true;
            $scope.loading = true;
            console.log({ player: 'X', computer: 'O', board: $scope.board, chance: $scope.chance});
    ...
    var has_won_vertical = function(mark) {
        for (var c = 0; c < 4; c++) {
            if ($scope.board[0][c] == mark && ($scope.board[0][c] == $scope.board[1][c]) && ($scope.board[0][c] == $scope.board[2][c]) && ($scope.board[0][c] == $scope.board[3][c])) {
    ...
    var has_won_horizontal = function(mark) {
        for (var r = 0; r < 4; r++) {
            if ($scope.board[r][0] == mark && ($scope.board[r][0] == $scope.board[r][1]) && ($scope.board[r][0] == $scope.board[r][2]) && ($scope.board[r][0] == $scope.board[r][3])) {
    ...
    var has_won_diagonal = function(mark) {
        if ($scope.board[0][0] == mark && ($scope.board[0][0] == $scope.board[1][1]) && ($scope.board[0][0] == $scope.board[2][2]) && ($scope.board[0][0] == $scope.board[3][3])) {
            return [[0, 0], [1, 1], [2, 2], [3,3]];
        }
        else if ($scope.board[3][0] == mark && ($scope.board[3][0] == $scope.board[2][1]) && ($scope.board[3][0] == $scope.board[1][2]) && ($scope.board[3][0] == $scope.board[0][3])) {
